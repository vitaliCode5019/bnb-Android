...
public class RaceModel extends CoordComparableModel {
    public String name;
    ...
    public static List<CoordComparableModel> parseFromHtml(String html) {
        ...
        for(int i = 1; i < nodes.size(); i++) {
            ...
            model.location = tr.select("td:nth-child(2)").text();
            model.url = tr.select("td:nth-child(3)>a").attr("href");
            model.startDate = tr.select("td:nth-child(4)").text();
            model.endDate = tr.select("td:nth-child(5)").text();
            model.type = tr.select("td:nth-child(7)").text();
            model.format = tr.select("td:nth-child(8)").text();
            model.notes = tr.select("td:nth-child(9)").text();
